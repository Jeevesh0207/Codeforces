Question Link :- (" https://codeforces.com/problemset/problem/1845/B ")
...
#define FOR(S, N) for (ll i = S; i < N; i++)
#define RFOR(S, N) for (ll i = N - 1; i >= S; i--)
...
ll CalHV(ll x,ll y, ll z){
  if(y>=x && z>=x) return min(y-x,z-x);
  if(y<=x && z<=x) return min(x-y,x-z);
  return 0;
}
...
void Rabbit()
{
  ...
  ll ans=CalHV(x1,x2,x3)+CalHV(y1,y2,y3)+1;
  cout<<ans<<endl; 
